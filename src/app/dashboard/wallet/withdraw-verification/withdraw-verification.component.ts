...
@Component({
  selector: 'app-withdraw-verification',
  templateUrl: './withdraw-verification.component.html',
  styleUrls: ['./withdraw-verification.component.css']
})
export class WithdrawVerificationComponent implements OnInit {
  url:any
  errormessage: any;
    fetching= false;
    successmessage: any;
  ...
  constructor(public api:DashboardService, public api1:AuthService, private activateRoute:ActivatedRoute,private formbuilder:FormBuilder,private router:Router) { }
  ...
  ngOnInit(): void {
    ...
    this.widId =this.party.id.id
    console.log("abc", this.party.id.id)
  ...
  AddOtp(){
    ...
    this.api.verifyOtpPhone(this.party.id.id,this.type,this.url).subscribe((res: any) => {
      this.fetching = true; 
  ...
  AddOtp1(){
    ...
    this.api.verifyOtpPhone(this.party.id.id,this.type1,this.url).subscribe((res: any) => {
      this.fetching = true; 
      // this.successmessage=res.message
      // console.log(this.successmessage)
      Swal.fire({
  ...
  submit(){
    this.fetching = true; 
    this.api.withdrawSubmit(this.addBroker.value).subscribe((res: any) => {
      this.fetching = true; 
      // this.successmessage=res.message
      // console.log(this.successmessage)
      Swal.fire({
